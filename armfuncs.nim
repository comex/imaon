    ...
    proc DIShift*(ctx : TAsmCtx, base : TRVal, typ, imm5 : TBinary) : TRVal =
        ...
        of 0b11:
            if imm5.num == 0:
                ...
                return ctx.Shift(base, TShiftKind.ROR, @@imm5)
        else: nil

    proc TEImm_C*(ctx : TAsmCtx, imm12 : TBinary) : tuple[ent : TRVal, setCarry : bool] =
        if imm12[11,10] == b"00":
            ...
            var r = ror(unrotated_value, imm12[11,7].num)
            result.ent = @@r
            result.setCarry = r[0].bit
